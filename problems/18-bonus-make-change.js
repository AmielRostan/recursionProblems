...
function greedyMakeChange(target, coins = [25, 10, 5, 1]) {
  ...
  for(let i of coins) {
    if(target >= i) {
      return [i].concat(greedyMakeChange(target - i, coins));
    }
  }
}

// console.log(greedyMakeChange(17));
...
function makeBetterChange(target, coins = [25, 10, 5, 1]) {
  ...
  for (let i = 0; i < coins.length; i++) {
    ...
    if (target >= coin) {
      const remainingChange = makeBetterChange(target - coin, coins.slice(i));

      if (remainingChange !== null && (bestChange === null || [coin, ...remainingChange].length < bestChange.length)) {
        bestChange = [coin, ...remainingChange];
      }
    }
  }
